  ...
  const dgbManageBind = async (instance) => {
    try {
      ...
      increaseButton.addEventListener("click", function () {
        don = parseInt(document.getElementById("priceID").value, 10);
        dgbInstance.methods.increase(don).send({ from: dgbID }).then(async function () {
      ...
      decreaseButton.addEventListener("click", function () {
        don = parseInt(document.getElementById("priceID").value, 10);
        dgbInstance.methods.decrease(don).send({ from: dgbID }).then(async function () {
          document.location.href = `/managemain`
        }).catch(function (err) {
          console.log(err.message);
        })
      ...
      confiscateButton.addEventListener("click", function () {
        don = parseInt(document.getElementById("priceC").value, 10);
        fromAcc = document.getElementById("companyId").value;
        dgbInstance.methods.confiscate(fromAcc, don).send({ from: dgbID }).then(async function (receipt) {
